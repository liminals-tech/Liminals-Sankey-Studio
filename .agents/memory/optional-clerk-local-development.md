...
The Sankey editor must remain public when no client Clerk publishable key is configured locally; authentication routes and controls are enabled only when the configured key resolves successfully. Production keeps the existing Clerk provider wiring; there is no backend API service or same-domain Clerk proxy — the browser talks directly to Clerk's Frontend API.
