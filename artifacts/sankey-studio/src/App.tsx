      ...
      <main className="min-w-0 flex-1 px-4 py-5 sm:px-7 sm:py-7">
        <div className="mx-auto max-w-[1160px]">
          ...
          <div className="mt-5 grid gap-2 rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card)/.45)] p-3 sm:grid-cols-[minmax(160px,1fr)_minmax(200px,1.7fr)]">
            <div><label htmlFor="chart-title" className="mb-1 block text-[10px] font-medium uppercase tracking-[.14em] text-[hsl(var(--muted-foreground))]">Title</label><input id="chart-title" value={title} onChange={(event) => setTitle(event.target.value)} className="w-full rounded border-0 bg-transparent px-0 text-sm font-semibold outline-none placeholder:text-[hsl(var(--muted-foreground))] focus-visible:ring-2 focus-visible:ring-[hsl(var(--primary))]" data-testid="input-chart-title" /></div>
            <div><label htmlFor="chart-subtitle" className="mb-1 block text-[10px] font-medium uppercase tracking-[.14em] text-[hsl(var(--muted-foreground))]">Subtitle / source note</label><input id="chart-subtitle" value={subtitle} onChange={(event) => setSubtitle(event.target.value)} className="w-full rounded border-0 bg-transparent px-0 text-xs text-[hsl(var(--muted-foreground))] outline-none placeholder:text-[hsl(var(--muted-foreground))] focus-visible:ring-2 focus-visible:ring-[hsl(var(--primary))]" data-testid="input-chart-subtitle" /></div>
          </div>
          <div className="mt-8 flex gap-3 text-[10px] text-[hsl(var(--muted-foreground))]"><button onClick={() => setLocation("/privacy")} className="hover:underline" data-testid="link-privacy">Privacy</button><span>·</span><button onClick={() => setLocation("/terms")} className="hover:underline" data-testid="link-terms">Terms</button></div>
        </div>
      </main>
