    ...
    <section className="min-h-[430px] flex-1 overflow-hidden rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-[var(--shadow-sm)]" data-testid="panel-sankey-visualization">
      ...
      <div className="studio-grid relative min-h-[355px] overflow-auto p-3 sm:p-5" style={{ backgroundColor: transparent ? "transparent" : background }}>
        {model.nodes.length < 2 ? <div className="grid min-h-[330px] place-items-center text-center"><div><p className="font-serif text-xl">Nothing to draw yet.</p><p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">Map at least two text columns and a numeric value.</p></div></div> : <svg viewBox="0 0 1000 560" className="sankey-animate mx-auto block h-auto min-w-0 transition-transform duration-200" style={{ width: `${zoom * 100}%`, minWidth: zoom > 1 ? "690px" : undefined }} role="img" aria-label={`Sankey diagram: ${title}`} data-testid="svg-sankey">
          <rect x="0" y="0" width="1120" height="560" fill="transparent" onClick={() => onSelect(null)} />
          {backgroundImage && <image href={backgroundImage} x="0" y="0" width="1120" height="560" preserveAspectRatio="xMidYMid slice" opacity=".16" pointerEvents="none" aria-label="Chart background image"><title>Chart background image</title></image>}
        ...
        <button onClick={() => setZoom(1)} className="absolute right-4 top-4 hidden items-center gap-1.5 rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--card)/.85)] px-2.5 py-1.5 text-[10px] text-[hsl(var(--muted-foreground))] backdrop-blur sm:flex" data-testid="button-fit-chart"><Maximize2 size={12} /> Fit</button>
      </div>
