...
export function SankeyCanvas({ model, title, subtitle, background, backgroundImage, transparent, showLabels, notation, linkOpacity, selectedId, onSelect, onResetLayout }: Props) {
  ...
  const totalDisplay = new Intl.NumberFormat("en", { maximumFractionDigits: 2 }).format(safe(model.total));
  const totalLabel = sourceNodes.length === 1 ? `${sourceNodes[0].label} total` : "Total flow";
