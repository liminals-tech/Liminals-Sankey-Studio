...
export function ExportMenu({ model, chartId, title, subtitle, background, backgroundImage, transparent, showLabels, notation, signedIn, isPrivate, onPrivateChange, onClose, onSaved }: Props) {
  ...
  const png = async (scale: number, isTransparent = false) => { try { await exportPng(model, { ...options, transparent: isTransparent || transparent }, scale, `sankey-${chartId}-${scale === 2 ? "2x-" : ""}${isTransparent ? "transparent-" : ""}export.png`); await onSaved(chartId); setDone(`${scale === 2 ? "2× PNG saved" : isTransparent ? "Transparent PNG saved" : "PNG saved"} · ${chartId}`); } catch { setDone("PNG export could not embed an image. Try a local image or export SVG."); } };
  const gif = async () => {
      ...
      <div className="space-y-3 p-5">
        ...
        <section className="rounded-lg border border-[hsl(var(--primary)/.35)] bg-[hsl(var(--primary)/.04)] p-3.5" aria-labelledby="gif-export-heading">
          ...
          <div className="mt-4 space-y-3">
             <label className="block"><span className="flex items-center justify-between text-[10px] font-medium"><span>Frame size</span><span className="font-mono text-[hsl(var(--primary))]">{gifSettings.width} × {gifSettings.height}</span></span><select value={gifFramePreset} onChange={(event) => updateGifFramePreset(event.target.value as GifFramePreset)} className="mt-2 w-full rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-2.5 py-2 text-[11px] outline-none focus:border-[hsl(var(--primary))]" aria-label="GIF frame size" data-testid="select-gif-frame-size">{gifFramePresets.map((preset) => <option key={preset.value} value={preset.value}>{preset.label}</option>)}</select><span className="mt-1 block text-[9px] text-[hsl(var(--muted-foreground))]">The Sankey fills the selected frame instead of leaving extra space below.</span></label>
