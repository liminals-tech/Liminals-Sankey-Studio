...
type GifSettings = { revealDurationMs: number; introHoldMs: number; outroHoldMs: number };
...
export function ExportMenu({ model, chartId, title, subtitle, background, backgroundImage, transparent, showLabels, notation, onClose, onSaved }: Props) {
  ...
  const [gifSettings, setGifSettings] = useState<GifSettings>({ revealDurationMs: 1800, introHoldMs: 1000, outroHoldMs: 1200 });
  ...
  const gif = async () => {
    ...
    setGifProgress(0);
    try {
      await exportGif(model, options, { ...gifSettings, onProgress: setGifProgress }, `sankey-${chartId}-animated.gif`);
      onSaved(chartId);
