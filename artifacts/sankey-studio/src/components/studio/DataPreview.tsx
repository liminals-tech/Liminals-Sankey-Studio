...
type Props = { rows: Row[]; columns: string[]; levels: string[]; valueColumn: string; summary?: ImportSummary; onImport: () => void };

export function DataPreview({ rows, columns, levels, valueColumn, summary, onImport }: Props) {
  ...
  return <section className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-[var(--shadow-xs)]" data-testid="panel-imported-data">
    ...
    <div className="border-b border-[hsl(var(--border))] bg-[hsl(var(--secondary)/.25)] px-5 py-2.5 sm:px-6"><p className="text-[10px] leading-relaxed text-[hsl(var(--muted-foreground))]"><strong className="text-[hsl(var(--foreground)/.8)]">Current reading:</strong> each row flows through <span className="font-mono text-[hsl(var(--foreground)/.75)]">{levels.join(" → ") || "no mapped levels"}</span>; <span className="font-mono text-[hsl(var(--foreground)/.75)]">{valueColumn || "no value column"}</span> controls its thickness.</p></div>
