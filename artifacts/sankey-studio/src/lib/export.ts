...
export function modelToSvg(model: SankeyModel, options: SankeyExportOptions) {
  ...
  const backgroundImage = options.backgroundImage ? `<image href="${escapeXml(options.backgroundImage)}" x="0" y="0" width="1120" height="560" preserveAspectRatio="xMidYMid slice" opacity=".16"/>` : "";
  ...
  const nodeImages = model.nodes.filter((node) => node.image).map((node) => `<image href="${escapeXml(node.image ?? "")}" x="${node.x - 5}" y="${node.y + Math.max(0, node.h / 2 - 14)}" width="${node.w + 10}" height="${Math.min(28, node.h)}" preserveAspectRatio="xMidYMid slice" opacity=".9"/>`).join("");
  return `<svg xmlns="http://www.w3.org/2000/svg" data-sankey-id="${chartId}" viewBox="0 0 1120 650">${metadata}<rect width="1120" height="650" fill="${bg}"/>${backgroundImage}<text x="90" y="35" fill="#243033" font-family="DM Sans, sans-serif" font-size="18" font-weight="600">${title}</text><text x="90" y="57" fill="#6a7372" font-family="DM Sans, sans-serif" font-size="12">${subtitle}</text><g>${model.links.map((link) => `<path d="${link.path}" fill="${link.source.color}" opacity=".3"/>`).join("")}</g><g>${model.nodes.map((node) => `<rect x="${node.x}" y="${node.y}" width="${node.w}" height="${node.h}" rx="3" fill="${node.color}"/>`).join("")}${nodeImages}</g><g>${labels}</g></svg>`;
}
