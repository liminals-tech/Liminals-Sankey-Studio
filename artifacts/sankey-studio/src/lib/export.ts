...
export function modelToSvg(model: SankeyModel, options: SankeyExportOptions) {
  ...
  const watermark = `<text x="1200" y="${(viewBoxHeight - 10).toFixed(1)}" fill="#6a7372" font-family="DM Sans, sans-serif" font-size="10" text-anchor="end">Created at liminals.tech</text>`;
  return `<svg xmlns="http://www.w3.org/2000/svg" data-sankey-id="${chartId}" width="${frameWidth}" height="${frameHeight}" viewBox="0 0 1220 ${viewBoxHeight}" preserveAspectRatio="xMidYMid meet">${metadata}<rect width="1220" height="${viewBoxHeight}" fill="${bg}"/>${backgroundImage}<text x="170" y="35" fill="#243033" font-family="DM Sans, sans-serif" font-size="18" font-weight="600">${title}</text><text x="170" y="57" fill="#6a7372" font-family="DM Sans, sans-serif" font-size="12">${subtitle}</text><g>${exportModel.links.map((link) => `<path d="${link.path}" fill="${link.source.color}" opacity="${(.3 * Math.min(revealAlpha(link.source.level), revealAlpha(link.target.level))).toFixed(3)}"/>`).join("")}</g><g>${exportModel.nodes.map((node) => `<rect opacity="${revealAlpha(node.level)}" x="${node.x}" y="${node.y}" width="${node.w}" height="${node.h}" rx="3" fill="${node.color}"/>`).join("")}${nodeImages}</g><g>${labels}</g>${watermark}</svg>`;
}
...
export async function exportPng(model: SankeyModel, options: SankeyExportOptions, scale = 1, filename = "sankey-studio.png") {
  ...
  const canvas = document.createElement("canvas"); canvas.width = 1220 * scale; canvas.height = 650 * scale;
...
export async function exportGif(model: SankeyModel, options: SankeyExportOptions, gifOptions: SankeyGifOptions, filename = "sankey-studio.gif") {
  const width = Math.min(4096, Math.max(320, Math.round(gifOptions.width)));
  const height = Math.min(4096, Math.max(320, Math.round(gifOptions.height)));
  ...
  const frameOptions = { ...options, transparent: false, frameWidth: width, frameHeight: height };
